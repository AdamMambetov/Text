...
for (let i = 0; i < dirs.length; i++) {
	const files = dirs[i].children
		...
		.filter(f => f.path.includes("Albums"))
	...
	for (let j = 0; j < files.length; j++) {
		...
		await tp.app.fileManager.processFrontMatter(file, async (fm) => {
			...
			if (fm["aliases"] == null)
				fm["aliases"] = []
			if (fm["tracklist"] == null)
				fm["tracklist"] = []
			
			if (fm["Creators"].length == 0
					&& fm["aliases"].length == 0
					&& fm["tracklist"].length == 0)
				return
			...
			let tracklist = ""
			for (let k = 0; k < fm["tracklist"].length; k++) {
				tracklist += `, "${fm["tracklist"][k]}"`
			}
			tracklist = `[${tracklist.substring(2, tracklist.length)}]`
			...
			await tp.app.vault.process(file, content => {
				...
				for (let k = 0; k < matches.length; k++) {
					...
					if (match[1] === "aliases"
							 || match[1] === "Creators"
							 || match[1] === "tracklist") {
						let value = match[1] == "aliases" ? aliases : match[1] == "Creators" ? creators : tracklist
