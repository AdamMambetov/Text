# Как узнать спецификатор доступа у функции из .T3D файла

1. Найти объявление функции `(Begin Object Name="<FUNCTION_NAME>")`
2. Внутри него есть параметр _ExtraFlags_, который является битовой маской

Значения можно посмотреть ниже или в исходниках Unreal Engine.
...
Взял из файла `Runtime/CoreUObject/Public/UObject/Script.h` строчка 130
 Perplexity сказал, где можно их найти.

Полный код перечисления `EFunctionFlags`:
